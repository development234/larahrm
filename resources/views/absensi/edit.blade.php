<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-6 border-b border-blue-400">
                        <h3 class="text-1xl font-bold">Edit Data Absensi</h3>
                    ...
                    <form action="{{ route('absensi.update', $absen->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select name="karyawan_id" id="karyawan_id" required
                                    ...
                                    @foreach($karyawan as $k)
                                        <option value="{{ $k->id }}" 
                                                ...
                                                {{ old('karyawan_id', $absen->karyawan_id) == $k->id ? 'selected' : '' }}>
                                            {{ $k->name_user }} - {{ $k->nik }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan', $absen->jabatan) }}" 
                                       readonly
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $absen->tanggal) }}" 
                                       required
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                @error('tanggal')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="Alpha" {{ old('status', $absen->status) == 'Alpha' ? 'selected' : '' }}>Alpha</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="time" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', $absen->jam_masuk) }}" 
                                       required
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                @error('jam_masuk')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="time" name="jam_keluar" id="jam_keluar" value="{{ old('jam_keluar', $absen->jam_keluar) }}" 
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                @error('jam_keluar')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div id="info-karyawan" class="mt-4 p-4 bg-blue-50 dark:bg-blue-900 rounded-lg {{ $absen->karyawan ? '' : 'hidden' }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    ...
                                    <span id="info-nik" class="text-blue-600 dark:text-blue-400 ml-2">
                                        {{ $absen->karyawan->nik ?? '-' }}
                                    </span>
                                </div>
                                <div>
                                    <span class="font-medium text-blue-700 dark:text-blue-300">Jabatan:</span>
                                    <span id="info-jabatan" class="text-blue-600 dark:text-blue-400 ml-2">
                                        {{ $absen->karyawan->jabatan ?? '-' }}
                                    </span>
                                </div>
                                <div>
                                    <span class="font-medium text-blue-700 dark:text-blue-300">Tanggal Gabung:</span>
                                    <span id="info-tanggal-gabung" class="text-blue-600 dark:text-blue-400 ml-2">
                                        @if($absen->karyawan && $absen->karyawan->tanggal_gabung)
                                            {{ \Carbon\Carbon::parse($absen->karyawan->tanggal_gabung)->format('d/m/Y') }}
                                        @else
                                            -
                                        @endif
                                    </span>
                        ...
                        <div class="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <h4 class="font-medium text-gray-700 dark:text-gray-300 mb-2">Informasi Absensi:</h4>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span class="font-medium text-gray-600 dark:text-gray-400">Dibuat Pada:</span>
                                    <span class="text-gray-500 dark:text-gray-300 ml-2">
                                        {{ $absen->created_at->format('d/m/Y H:i') }}
                                    </span>
                                </div>
                                <div>
                                    <span class="font-medium text-gray-600 dark:text-gray-400">Diupdate Pada:</span>
                                    <span class="text-gray-500 dark:text-gray-300 ml-2">
                        ...
                        <div class="mt-6 flex justify-end space-x-3">
                            <a href="{{ route('absensi.index') }}" class="px-6 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:bg-gray-600 dark:text-gray-300 dark:border-gray-500">
                                Batal
                            </a>
    ...
    <script>
        ...
        document.getElementById('jam_masuk').addEventListener('change', function() {
            ...
            // Only auto-calculate if jam_keluar is empty or default
            if (jamMasuk && (!jamKeluar || jamKeluar === '17:00')) {
                const [hours, minutes] = jamMasuk.split(':');
                let endHours = parseInt(hours) + 9; // Default 9 jam kerja
                if (endHours >= 24) endHours -= 24;
                
                const newJamKeluar = String(endHours).padStart(2, '0') + ':' + minutes;
                document.getElementById('jam_keluar').value = newJamKeluar;
            }
        });
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            // Validasi untuk status selain Hadir
            if (status !== 'Hadir' && jamMasuk) {
                if (!confirm('Status bukan "Hadir" tetapi jam masuk diisi. Apakah Anda yakin?')) {
                    e.preventDefault();
                    return;
                }
            }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Set initial values for info karyawan jika ada data lama
            const initialKaryawanId = document.getElementById('karyawan_id').value;
            if (initialKaryawanId && karyawanData[initialKaryawanId]) {
                const karyawan = karyawanData[initialKaryawanId];
                document.getElementById('info-nik').textContent = karyawan.nik;
                document.getElementById('info-jabatan').textContent = karyawan.jabatan;
                document.getElementById('info-tanggal-gabung').textContent = new Date(karyawan.tanggal_gabung).toLocaleDateString('id-ID');
            }
        });
    </script>
</x-app-layout>