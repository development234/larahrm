<x-app-layout>
    <x-slot name="header">
        <h5 class="font-semibold text-lg text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Ajukan Perizinan') }}
        </h5>
    </x-slot>
        ...
        <div class="max-w-7xl sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-1xl font-bold mb-6">Form Pengajuan Perizinan</h3>
                    <hr class="border-blue-400 mt-1 mb-3">
                    <form action="{{ route('perizinan.store') }}" method="POST">
                        @csrf
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Karyawan - Dropdown dari database -->
                            <div>
                            ...
                            <!-- Jabatan (akan terisi otomatis via JavaScript) -->
                            <div>
                            ...
                            <div>
                                ...
                                <select name="jenis_izin" id="jenis_izin" 
                                    ...
                                    <option value="Cuti Tahunan" {{ old('jenis_izin') == 'Cuti Tahunan' ? 'selected' : '' }}>Cuti Tahunan</option>
                                    <option value="Cuti Sakit" {{ old('jenis_izin') == 'Cuti Sakit' ? 'selected' : '' }}>Cuti Sakit</option>
                                    <option value="Izin Pribadi" {{ old('jenis_izin') == 'Izin Pribadi' ? 'selected' : '' }}>Izin Pribadi</option>
                                    <option value="Izin Keluarga" {{ old('jenis_izin') == 'Izin Keluarga' ? 'selected' : '' }}>Izin Keluarga</option>
                                    <option value="Izin Lainnya" {{ old('jenis_izin') == 'Izin Lainnya' ? 'selected' : '' }}>Izin Lainnya</option>
                                </select>
                                @error('jenis_izin')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="durasi" id="durasi" 
                                    ...
                                    <option value="2 Minggu" {{ old('durasi') == '2 Minggu' ? 'selected' : '' }}>2 Minggu</option>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Data karyawan dari PHP - pastikan formatnya sesuai
        ...
        // Trigger change event jika ada nilai old
        @if(old('karyawan_id'))
